    ...
    >
      ...
      >
        {laid.map(({ event, column, columnCount }) => {
          ...
          const rawDurationMin = (end.getTime() - start.getTime()) / 60000;
          const maxRemainingMin = HOURS_IN_DAY * 60 - startMin;
          const durationMin = Math.min(rawDurationMin, maxRemainingMin);
          const height = Math.min(Math.max(durationMin, MIN_EVENT_HEIGHT), maxRemainingMin) * PX_PER_MIN;
