...
- **Collapsed appearance:** The sidebar shrinks to an 18px-wide strip containing only ▶ centered vertically. Clicking anywhere on the strip expands the sidebar.
...
- **Animation:** CSS `width` transition, 200ms ease, on `.m365-calendar-selector`. Inner content uses `overflow: hidden` to clip cleanly during the animation.
...
### `styles.css`
- `.m365-calendar-selector`: add `transition: width 200ms ease; overflow: hidden;`.
